...
tabs_titles_2= ["Estudo Evasão de alunos na Passos","Modelo","Indicadores", "Análise do Estudo"]
...
with tabs_2[0]: 
    ...
    with colunas_3 [0]:
        ...
        st.markdown('<p style="text-align: justify;">Essas quatro pedras, que simbolizam etapas de uma jornada de aprendizado e de desenvolvimento educacional, indicarão a posição de cada estudante em relação ao desempenho geral de todos na pesquisa avaliativa PEDE 2021.<br><br></p>', unsafe_allow_html = True)
        st.image("images/pedraINDE.png",caption="Faixas de desempenho Pedra-conceito INDE - Fonte:PEDE Pontos importantes", width=500)	
        st.markdown('<p style="text-align: justify;">Analisando a evasão por Pedra, é possível identificar que a pedra Ametista e Ágata, são as que mais possuem alunos na situação de evasão, onde há alunos com valores na média do INDE.<br><br></p>', unsafe_allow_html = True)
        st.image("images/ev_pedra.png",caption="Evasão por Pedra", width=500)	
...
with tabs_2[2]: 
    ...
    st.markdown('''<p style='text-align: justify;'> Os dados abaixo mostram de uma forma mais clara, sobre os indicadores utilizados para o estudo e a análise.
                <br><br>
                </p>''',unsafe_allow_html=True)
